# Data Contracts

## War Snapshot Files

Final war snapshots are raw Clash `currentwar` responses saved as JSON.

Default locations:

```text
data/wars/war_YYYY-MM-DD_HH-MM.json
data/current_war/latest_current_war.json
data/war_results/final_war_YYYY-MM-DD_HH-MM.json
data/state/saved_wars.json
site_output/index.html
site_output/current-war.html
site_output/history.html
```

`data/wars/` contains manual snapshots from `fetch_war.py`.

`data/current_war/latest_current_war.json` contains the latest current-war snapshot for static current-war page generation. It is runtime data and should not be committed.

`data/war_results/` contains final snapshots from `schedule_war_snapshot.py` and is the default input for `weekly_report.py`.

`data/state/saved_wars.json` tracks dedupe keys so the scheduler does not save the same ended war repeatedly.

`site_output/index.html` is generated from saved final snapshots and is intended to be committed for Cloudflare Pages deployment. It is not runtime data.

`site_output/current-war.html` is optionally generated by `build_site.py --include-current-war` from `data/current_war/latest_current_war.json`, or from one live `currentwar` API response when `--live-current-war-fallback` is used. It is a static build artifact and does not contain the API token.

`site_output/history.html` is generated from all deduped saved final snapshots in `data/war_results/`. It is a static build artifact and does not call the Clash API.

## Fields Used From Clash War JSON

Top-level fields:

```text
state
preparationStartTime
startTime
endTime
attacksPerMember
clan
opponent
```

Clan fields:

```text
clan.tag
clan.name
clan.stars
clan.destructionPercentage
clan.members
```

Opponent fields:

```text
opponent.tag
opponent.name
opponent.stars
opponent.destructionPercentage
```

Member fields:

```text
member.tag
member.name
member.attacks
```

Attack fields:

```text
attack.stars
```

## Required Fields By Script

`fetch_war.py` can save any valid JSON response from the API, but its participation printout expects `clan.members`.

`schedule_war_snapshot.py` uses `state` and `endTime` for timing. It uses `clan.tag`, `opponent.tag`, `preparationStartTime`, `startTime`, and `endTime` for final-save dedupe.

`war_warning_message.py` requires `state == "inWar"` before generating a reminder. It uses `endTime`, `attacksPerMember`, and `clan.members`.

`weekly_report.py` uses `startTime`, `clan.tag`, and `opponent.tag` for dedupe and filtering. It uses clan/opponent stars, member attacks, attack stars, and optional destruction percentage for metrics.

`build_site.py` uses the weekly report output and writes escaped dashboard HTML into `site_output/index.html`. It also writes all-time saved-snapshot history to `site_output/history.html`. With `--include-current-war`, it uses the latest current-war snapshot to write `site_output/current-war.html`; if current-war data is unavailable, it writes a static fallback page.

## Derived Fields

Remaining attacks:

```text
remaining = max(0, attacksPerMember - len(member.attacks))
```

Total used attacks:

```text
sum(len(member.attacks) for each clan member)
```

Total possible attacks:

```text
sum(attacksPerMember for each clan member)
```

Total stars:

```text
sum(attack.stars for each attack by each clan member)
```

War record:

```text
clan.stars > opponent.stars  -> win
clan.stars < opponent.stars  -> loss
same stars                  -> tie
```

Member accountability:

```text
wars_participated += 1 when member appears in war.clan.members
possible_attacks += attacksPerMember
attacks_used += len(member.attacks)
attacks_missed += max(0, attacksPerMember - len(member.attacks))
total_stars += sum(attack.stars for each member attack)
destruction_sample_count = count(member attacks with destructionPercentage)
avg_destruction = sum(member attack destructionPercentage) / destruction_sample_count
miss_rate = attacks_missed / possible_attacks
```

## Assumptions

- Clash times use `YYYYMMDDTHHMMSS.000Z` and are parsed as UTC-aware datetimes.
- Site display times are converted to Central Time with `ZoneInfo("America/Chicago")` when available, with UTC as fallback.
- `attacksPerMember` defaults to 2 if missing or invalid.
- `member.attacks` may be missing; missing attacks are treated as an empty list.
- `clan.members` may be missing; missing members are treated as an empty list.
- Missing player names are displayed as `Unknown`.
- Malformed weekly report input files are skipped with a short message.
- The weekly report only counts snapshots with `startTime`, `clan.tag`, and `opponent.tag`.
