...
`build_site.py` uses the weekly report output and writes escaped dashboard HTML into `site_output/index.html`. It also writes all-time saved-snapshot history to `site_output/history.html`. With `--include-current-war`, it uses a live `currentwar` response to write `site_output/current-war.html`; if the live response is unavailable, it writes a static fallback page.
...
```text
wars_participated += 1 when member appears in war.clan.members
possible_attacks += attacksPerMember
attacks_used += len(member.attacks)
attacks_missed += max(0, attacksPerMember - len(member.attacks))
total_stars += sum(attack.stars for each member attack)
destruction_sample_count = count(member attacks with destructionPercentage)
avg_destruction = sum(member attack destructionPercentage) / destruction_sample_count
miss_rate = attacks_missed / possible_attacks
```
